...
public class PluginManager
{
    ...
    public bool LoadPluginConstants(PluginInfo plugin)
    {
        try
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (colonPos > 0)
                {
                    var constantName = trimmed.Substring(0, colonPos).Trim();
                    var remainder = trimmed.Substring(colonPos + 1);
                    
                    // Remove any inline comments (after semicolon)
                    var commentPos = remainder.IndexOf(';');
                    var valueStr = commentPos >= 0 
                        ? remainder.Substring(0, commentPos).Trim() 
                        : remainder.Trim();
    ...
    /// </summary>
    public List<PluginInfo> GetDiscoveredPlugins() => _discoveredPlugins;
    ...
    private bool TryParseConstantValue(string valueStr, out int value)
    {
        value = 0;
        ...
        {
            return int.TryParse(valueStr.Substring(1), System.Globalization.NumberStyles.HexNumber, null, out value);
        }
        if (valueStr.StartsWith("0x", StringComparison.OrdinalIgnoreCase))
        {
            return int.TryParse(valueStr.Substring(2), System.Globalization.NumberStyles.HexNumber, null, out value);
        }
