...
public class DefineManager
{
    ...
    {
        return _userDefines.TryGetValue(name, out value);
    }
    ...
    {
        return _pluginDefines.TryGetValue(pluginName, out pluginId);
    }
    
    /// <summary>
    /// Get the current active plugin ID (last referenced plugin)
    /// </summary>
    public int GetActivePluginId(string constantName)
    {
        // Check if constant name suggests a plugin (this is a simplified version)
        // In VB6, this is tracked by Glob.PluginLastID
        // For now, return 0 (no active plugin)
        return 0;
    }
}
