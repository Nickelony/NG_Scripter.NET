...
public class ScriptParser
{
    ...
    private bool _languageFileLoaded = false;
    ...
    public bool ParseScriptFile(string scriptPath, string trleFolderPath, out ScriptData? scriptData)
    {
        ...
        if (constantsPath is null && !string.IsNullOrEmpty(trleFolderPath))
        {
        ...
        if (constantsPath is not null)
        {
            ...
            _ngParser.SetConstantsLoader(_constantsLoader);
        }
        else
        {
        ...
        if (!string.IsNullOrEmpty(trleFolderPath))
        {
            ...
            if (_objectsHParser.LoadObjectsH(objectsHPath))
            {
                _ngParser.SetObjectsHParser(_objectsHParser);
            }
        }
        
        // Pass define manager to NG parser
        _ngParser.SetDefineManager(_defineManager);
        ...
        if (!string.IsNullOrEmpty(trleFolderPath))
        {
            ...
            // Pass plugin manager to NG parser for constant resolution
            _ngParser.SetPluginManager(_pluginManager);
        }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                while (true)
                {
                    ...
                    if (continuationPos >= 0 && (commentPos < 0 || continuationPos < commentPos))
                    {
                        ...
                        rawLine = rawLine.Substring(0, continuationPos).Trim();
                ...
                if (newSection != ScriptSectionType.None)
                {
                    ...
                    {
                        LoadFirstLanguageFile(Path.GetDirectoryName(scriptPath) ?? "");
                    }
                    
                    if (newSection == ScriptSectionType.Level || newSection == ScriptSectionType.Title)
                    {
                ...
                else if (currentSection == ScriptSectionType.PCExtensions || 
                         currentSection == ScriptSectionType.PSXExtensions)
                {
                    if (!ProcessExtensionCommand(currentSection, command, arguments, lineNumber))
                        return false;
                }
                else if (currentSection == ScriptSectionType.Level || 
                         currentSection == ScriptSectionType.Title)
                {
    ...
    {
        // Classic Options commands
        if (command is "LoadSave=" or "Title=" or "PlayAnyLevel=" or "FlyCheat=" or 
            "DemoDisc=" or "InputTimeOut=" or "Security=")
            return false;
        
        // Classic Language/Extension commands
        if (command is "File=" or "Level=" or "Cut=" or "FMV=" or "Name=")
            return false;
    ...
    private void RegisterPluginCommand(List<string> arguments, int lineNumber)
    {
        ...
        if (plugin is null)
        {
            ...
            string trlePath = Path.GetDirectoryName(Path.GetDirectoryName(_scriptData.IncludeStack[0].FileName)) ?? "";
