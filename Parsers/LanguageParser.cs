...
public class LanguageParser
{
    ...
    public LanguageParser(CompilerGlobals globals)
    {
        _globals = globals;
    }
    ...
    public bool ParseLanguageFile(string filePath, out LanguageData? languageData)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            if (normalizedLine.StartsWith("[") && normalizedLine.EndsWith("]"))
            {
                ...
                else if (sectionName == "psxstrings" || sectionName == "psx strings")
                {
                    currentSection = 1;
                    sectionSize = 0;
                }
                else if (sectionName == "pcstrings" || sectionName == "pc strings")
                {
                    currentSection = 2;
                    sectionSize = 0;
                }
                else if (sectionName == "extrang" || sectionName == "extra_ng")
                {
                    foundExtraNG = true;
                    currentSection = -1;
                }
            ...
            if (foundExtraNG)
            {
                ...
                if (colonIndex > 0)
                {
                    if (int.TryParse(normalizedLine.Substring(0, colonIndex).Trim(), out int index))
                    {
                        string text = normalizedLine.Substring(colonIndex + 1).Trim();
            ...
            else if (currentSection >= 0)
            {
                ...
                string dummy = string.Empty;
                StringUtilities.ConvertCStyleToActual(ref text, out dummy);
                ...
                if (currentSection == 0) _languageData.TotalStrings++;
                else if (currentSection == 1) _languageData.TotalPSXStrings++;
                else if (currentSection == 2) _languageData.TotalPCStrings++;
            }
        }
    ...
    /// </summary>
    public int GetStringIndex(LanguageData languageData, string text)
    {
