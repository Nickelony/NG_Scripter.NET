...
public class NGParser
{
    ...
    private Dictionary<string, NGCommandDefinition> _commandDefinitions = new();
    private LanguageData? _languageData;
    private PluginManager? _pluginManager;
    private ConstantsLoader? _constantsLoader;
    private ObjectsHParser? _objectsHParser;
    private DefineManager? _defineManager;
    private ExpressionEvaluator? _expressionEvaluator;
    private int _lastPluginId = 0; // Track plugin ID for Customize/Parameters commands
    ...
    public void SetConstantsLoader(ConstantsLoader constantsLoader)
    {
        _constantsLoader = constantsLoader;
    }
    
    public void SetObjectsHParser(ObjectsHParser objectsHParser)
    {
        _objectsHParser = objectsHParser;
    }
    
    public void SetDefineManager(DefineManager defineManager)
    {
        _defineManager = defineManager;
    }
    ...
    public NGCommand? ParseNGCommand(string commandName, List<string> arguments, int lineNumber, bool isOptionsCommand)
    {
        ...
        if (!ParseArguments(definition, arguments, command, lineNumber))
        {
            return null;
        }
        ...
        {
            if (TryOptimizeTriggerGroup(command, out var optimizedDefinition))
            {
    ...
    private bool ParseArguments(NGCommandDefinition definition, List<string> arguments, NGCommand command, int lineNumber)
    {
        ...
        bool hasVariableArray = definition.ArgumentTypes.Any(t => 
            t == NGArgumentType.Array || 
            t == NGArgumentType.ArrayByte || 
            t == NGArgumentType.ArrayNybble ||
            t == NGArgumentType.ArrayLong);
        ...
        for (int i = 0; i < fixedArgCount; i++)
        {
            ...
            if (!ParseArgument(definition.ArgumentTypes[i], arguments[i], command, lineNumber))
            {
                return false;
            }
            ...
            if (i == 0 && ((int)definition.Code == 20 || (int)definition.Code == 27) && definition.ArgumentTypes[i] == NGArgumentType.Long && _lastPluginId > 0)
            {
                ...
                int valueWithPlugin = originalValue | (_lastPluginId << 16);
                command.Arguments[0] = valueWithPlugin;
            }
            ...
            if (i == 1 && (int)definition.Code == 1 && definition.ArgumentTypes[i] == NGArgumentType.Long && _lastPluginId > 0)
            {
                ...
                int valueWithPlugin = originalValue | (_lastPluginId << 16);
                command.Arguments[1] = valueWithPlugin;
            }
        }
        ...
        if (hasVariableArray)
        {
            ...
            for (int i = fixedArgCount; i < arguments.Count; i++)
            {
                ...
                if (arrayType == NGArgumentType.ArrayNybble)
                {
                    ...
                    if (!ParseArgument(NGArgumentType.Word, arguments[i], tempCommand, lineNumber))
                    {
                        return false;
                    }
                ...
                {
                    if (!ParseArgument(elementType, arguments[i], tempCommand, lineNumber))
                    {
                        return false;
                    }
    ...
    private bool ParseArgument(NGArgumentType type, string argument, NGCommand command, int lineNumber)
    {
        try
        {
            switch (type)
    ...
    private int GetArgumentWordCount(object arg, NGArgumentType type)
    {
        switch (type)
        {
            case NGArgumentType.Word:
            ...
            case NGArgumentType.ArrayByte:
                if (arg is List<object> byteArray)
                {
                    ...
                    if ((totalBytes & 1) != 0) totalBytes++; // Pad to even
                    return totalBytes / 2;
                }
                return 0;
        ...
        switch (type)
        {
            ...
            case NGArgumentType.ArrayByte:
                if (arg is List<object> byteArray)
                {
                    ...
                    bytes.Add((byte)byteArray.Count); // Count first
            ...
            case NGArgumentType.ArrayNybble:
                if (arg is List<object> nibbleArray)
                {
                    ...
                    var bytes = new List<byte>();
                    bytes.Add((byte)nibbleArray.Count); // Count first
    ...
    private int GetStringIndex(string text)
    {
        ...
        if (text.StartsWith("#") && int.TryParse(text.Substring(1), out int directIndex))
        {
            return directIndex;
        }
        
        if (text.StartsWith("!") && int.TryParse(text.Substring(1), out int ngIndex))
        {
            return 32768 + ngIndex;
        }
        
        if (text.StartsWith("&") && int.TryParse(text.Substring(1), System.Globalization.NumberStyles.HexNumber, null, out int binIndex))
        {
            return binIndex;
        }
